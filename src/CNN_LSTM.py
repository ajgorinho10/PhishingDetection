...
class CNN_LSTM(nn.Module):
    def __init__(self, first_layer = 8, max_len = 100):
        ...
        self.conv = nn.Conv1d(
            ...
            out_channels=first_layer*2, 
            kernel_size=3,
            padding='same'
            )#wyjscie to [128,64,100]
        
        self.gelu = nn.GELU()
        self.pool = nn.MaxPool1d(kernel_size=2)#po maxpool to [128,64,50]
        
        
        #Warstwa LSTM
        self.lstm = nn.LSTM(
            input_size=first_layer*2,
    ...
    def train_model(self, X, y, epochs, save_model = False):
        ...
        optimalizer = torch.optim.AdamW(self.parameters(), lr = 0.002)
        loss_f = nn.BCELoss()
        ...
        if save_model:
           torch.save(self.state_dict(),"models/CNN_LSTM_v1.pth")
                
        
    def predicts(self):
     
        self.load_state_dict(torch.load("models/CNN_LSTM_v1.pth"))
        #self.to(self.device)
        self.eval()
        with torch.no_grad():
            user_input = None
            while user_input != "exit":
                user_input = input("Podaj url:")
                url_tokenized = self.tokenize([user_input])
                url = torch.tensor(url_tokenized)
...
if __name__ == "__main__":
    ...
    print("Ilość adresów url: ",data.df['url'].count())
    print("Ilość etykiet dla url: ",data.df['label'].count())
    

    lstm = CNN_LSTM(first_layer=32)

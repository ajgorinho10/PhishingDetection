...
class CnnLstm2(nn.Module, BaseModel):
    def __init__(self, cfg):
        ...
        self.cnn_proj = nn.Sequential(
            nn.Linear(in_features=self.conv_out_dim, out_features=self.conv_out_dim//4),
            nn.ReLU(),
            nn.Dropout(self.cfg.DROPOUT * 0.5),
        )
        
        self.lstm = nn.LSTM(
            input_size      = self.conv_out_dim//4,
        ...
        self.lstm_out_dim = self.cfg.LSTM_UNITS * 4
    ...
    def forward(self, x, return_attention: bool = False):
        ...
        # True GAP
        lstm_out_masked_avg = lstm_out.masked_fill(mask_expanded, 0.0)
        valid_lengths = (~mask).sum(dim=1, keepdim=True).clamp(min=1)
        gap = lstm_out_masked_avg.sum(dim=1) / valid_lengths
        
        # True GMP
        lstm_out_masked_max = lstm_out.masked_fill(mask_expanded, -1e9)
        gmp = lstm_out_masked_max.max(dim=1)[0]
        
        context = torch.cat([gap, gmp], dim=1)
